 ...
 * MACROS
 */
#if !defined( JOIN_NEXT_TURN_DELAY )
  #define JOIN_NEXT_TURN_DELAY       1000
#endif
...
CONST uint8 ZDO_Config_NvRestore = TRUE;
#else 
CONST uint8 ZDO_Config_NvRestore = FALSE;
#endif

CONST uint16 ZDO_NWK_START_DELAY = NWK_START_DELAY;

CONST uint16 ZDO_BEACON_REQUEST_DELAY = BEACON_REQUEST_DELAY;

CONST uint16 ZDO_JOIN_NEXT_TURN_DELAY = JOIN_NEXT_TURN_DELAY;

#if (defined MT_ZDO_CB_FUNC)
CONST uint8 ZDO_Config_MT_En = TRUE;
#else
CONST uint8 ZDO_Config_MT_En = FALSE;
#endif
 ...
 * @fn      ZDP_DirectCB()
 *
 * @brief   ZDO direct callback to MT
 *
 * @param   pData - Incoming AF frame
 *
 * @return  none
 */
void ZDP_DirectCB(afIncomingMSGPacket_t *pData, zdoIncomingMsg_t *inMsg)
{
#if (defined MT_ZDO_CB_FUNC)
#if !defined MT_TASK
  if (zgZdoDirectCB)
